...
//		}
		...
		public static void deleteDublicates(ArrayList<edge> newEdges)
		{
			for(int i=0; i<newEdges.size()-1; i++)
			{
				if(newEdges.get(i).equals(newEdges.get(i+1))==true)
				{
					newEdges.remove(i);
					i--;
				}
		...
		public void removeNodeInMSTVersion2(node n, Graph grafos,ArrayList<edge>newEdges )
		{
			Graph.insertionSort(newEdges);
			deleteDublicates(newEdges);
		for(int i=0; i<grafos.getMst().getMedges().size(); i++)
		{
			if(grafos.getMst().getMedge(i).isOneOfNodes(n)==false)
			{
				newEdges.add(grafos.getMst().getMedge(i));
			}
		}
